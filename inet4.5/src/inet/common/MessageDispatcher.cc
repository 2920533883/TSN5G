...
cGate *MessageDispatcher::handlePacket(Packet *packet, cGate *inGate)
{
    ...
    if (strcmp(packet->getName(), "besteffort") == 0){
        EV << "-------------- made by zrf --------------" << endl;
        EV << packet->getName() << endl;
        EV << "-------------- made by zrf --------------" << endl;
        auto outGate = gate("out", 2);
        return outGate;
    }
    if (strcmp(packet->getName(), "video") == 0){
            EV << "-------------- made by zrf --------------" << endl;
            EV << packet->getName() << endl;
            EV << "-------------- made by zrf --------------" << endl;
            auto outGate = gate("out", 2);
...
cGate *MessageDispatcher::handleMessage(Message *message, cGate *inGate)
{
    ...
    if (interfaceReq != nullptr) {
        int interfaceId = interfaceReq->getInterfaceId();
        for (auto a: interfaceIdToGateIndex){
            EV << a.first << "  " << a.second << endl;
        }
        EV << interfaceId << endl;
        auto it = interfaceIdToGateIndex.find(interfaceId);
        if (it != interfaceIdToGateIndex.end()) {
